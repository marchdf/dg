...
void simpleMesh::buildCombined(int order, const fullMatrix<scalar> &XYZNodesF, const int D)
{
  // Objective: match the interfaces with their farfield partner (in y)
  //            and periodic in y
  ...
  for(int c1 = 0; c1 < _N_B; c1++){
    ...
    for(int c2 = 0; c2 < _N_B; c2++){
      ...
      if ((fabs(xcen1-xcen2)<1E-6)&&(fabs(ycen1-ycen2)+1E-6 >= H)){_boundary[c1*2+0] = t1; _boundary[c1*2+1] = t1;}// t1=t2
      if ((fabs(ycen1-ycen2)<1E-6)&&(fabs(xcen1-xcen2)+1E-6 >= L)){_boundary[c1*2+0] = t1; _boundary[c1*2+1] = t2;}// t1=t2
    }
  }
