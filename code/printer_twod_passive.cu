/*!
  \file printer_twod_passive.cu
  \brief Kernel to output 2D passive solution used by the PRINTER class
...
arch_global void formater(int N_s, int N_E, scalar* U, scalar* output, bool inverse){
  ...
  */
  ...
  if(!inverse){
	...
	scalar phic  = U[(e*N_F+4)*N_s+i]/rho;
	scalar phinc = U[(e*N_F+5)*N_s+i];
	scalar gamma = constants::GLOBAL_GAMMA;
    
	output[(e*N_F+0)*N_s+i] = rho;
	output[(e*N_F+1)*N_s+i] = ux;
	output[(e*N_F+2)*N_s+i] = uy;
	output[(e*N_F+3)*N_s+i] = (gamma-1)*(et - 0.5*(ux*ux+uy*uy)*rho);
	output[(e*N_F+4)*N_s+i] = phic;
	output[(e*N_F+5)*N_s+i] = phinc;
      } // loop on nodes
    }  // loop on elements
  } // if inverse
  ...
  //
	...
	scalar phic  = output[(e*N_F+4)*N_s+i];
	scalar phinc = output[(e*N_F+5)*N_s+i];
	scalar gamma = constants::GLOBAL_GAMMA;
	...
	U[(e*N_F+4)*N_s+i] = rho*phic;
	U[(e*N_F+5)*N_s+i] = phinc;
