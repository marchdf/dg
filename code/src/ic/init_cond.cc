...
// Check how important delta is (otherwise same as rmawave)
void init_dg_deltchk_stiffened(const int N_s, const int N_E, const fullMatrix<scalar> &XYZNodes, const fullMatrix<scalar> &XYZCen, fullMatrix<scalar> &U, const std::vector<double> &ic_inputs){
  ...
  scalar Wratio = ic_inputs[3]; // Wavewidth to interface wavelength ratio
  scalar vcoord = ic_inputs[4]; //140;//111;//51.5;//134;//72.9; // coordinate shift upwards
  scalar itcoef = ic_inputs[5]; // Interface thickness (delta) multiplier
  scalar Rratio = 0;

  if (Wtype==2) {
    Rratio = ic_inputs[6]; //Ratio of wave rise and fall width of wavelength (included in Wratio)
  }

  

  printf("Wtype=%f, pa=%f, Aratio=%f, Wratio=%f, vcoord=%f, itcoef=%f\n",Wtype,pa,Aratio,Wratio,vcoord,itcoef);
