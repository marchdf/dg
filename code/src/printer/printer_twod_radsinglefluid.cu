/*!
  \file printer_twod_radsinglefluid.cu
  \brief Kernel to output 2D Reisner AD singlefluid solution used by the PRINTER class
...
arch_global void formater(int N_s, int N_E, scalar* U, scalar* output, bool inverse){
  ...
  */
  ...
  if(!inverse){
	...
	output[(e*N_F+4)*N_s+i] = U[(e*N_F+4)*N_s + i];
	output[(e*N_F+5)*N_s+i] = U[(e*N_F+5)*N_s + i];
  ...
  //
	...
	U[(e*N_F+4)*N_s+i] = output[(e*N_F + 4)*N_s+i];
	U[(e*N_F+5)*N_s+i] = output[(e*N_F + 5)*N_s+i];
