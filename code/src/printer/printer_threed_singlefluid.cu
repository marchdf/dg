/*!
  \file printer_threed_singlefluid.cu
  \brief Kernel to output 3D singlefluid solution used by the PRINTER class
  \copyright Copyright (C) 2012-2015, Regents of the University of Michigan
  \license This project is released under the GNU Public License. See LICENSE.
  \author Marc T. Henry de Frahan <marchdf@umich.edu> and Philip E. Johnson <phedjohn@umich.edu>, Computational Flow Physics Laboratory, University of Michigan
  \ingroup printer
*/
#ifdef THREED
#ifdef SINGLEFLUID
...
arch_global void formater(int N_s, int N_E, scalar* U, scalar* output, bool inverse){
  ...
  */
  ...
  if(!inverse){
	...
	scalar uz =  U[(e*N_F+3)*N_s+i]/rho;
	scalar et  = U[(e*N_F+4)*N_s+i];
	scalar gamma = constants::GLOBAL_GAMMA;
	
	output[(e*N_F+0)*N_s+i] = rho;
	output[(e*N_F+1)*N_s+i] = ux;
	output[(e*N_F+2)*N_s+i] = uy;
	output[(e*N_F+3)*N_s+i] = uz;
	output[(e*N_F+4)*N_s+i] = (gamma-1)*(et - 0.5*(ux*ux + uy*uy + uz*uz)*rho);
  ...
  //
	...
	scalar uz    = output[(e*N_F+3)*N_s+i];
	scalar gamma = constants::GLOBAL_GAMMA;
	...
	U[(e*N_F+3)*N_s+i] = rho*uz;
	U[(e*N_F+4)*N_s+i] = p/(gamma-1) + 0.5*rho*(ux*ux + uy*uy + uz*uz);
      } // loop on nodes
    }  // loop on elements
  } // if inverse
}
