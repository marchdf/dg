/*!
  \file printer_oned_radsinglefluid.cu
  \brief Kernel to output 1D Reiender AD singlefluid solution used by the PRINTER class
...
arch_global void formater(int N_s, int N_E, scalar* U, scalar* output, bool inverse){
  ...
  */
  ...
  if(!inverse){
	...
	output[(e*N_F+3)*N_s+i] = U[(e*N_F+3)*N_s+i];
