...
class polynomialBasis
{
 public:
  ...
  inline void f(fullMatrix<double> &coord, fullMatrix<double> &sf) const
  {
    ...
    //       printf("Inside the basis 'f' function: resized sf to %d x %d\n", coord.size1(), coefficients.size1());
    for (int iPoint=0; iPoint< coord.size1(); iPoint++) {
	  ...
	  //  printf("set sf(ipoint=%d, i=%d) = %f\n", iPoint, i, sf(iPoint, i));
	}
    }
  }
  // I added this
  inline void df(double u, double v, double w, fullMatrix<double> &sdf) const
  {
    double grads[coefficients.size1()][3];
    sdf.resize (coefficients.size1(),3);
    df(u,v,w,grads);
    for(int i = 0; i<coefficients.size1();i++){
      for(int j =0; j<3; j++){
	sdf(i,j) = grads[i][j];
