//10/23/2016: gmsh data translator for 2D calculations.
...
const int NL = 200; //resolution of post-processed solution line
const int N = 2; //DG discretization order (like, p1,p2,...)
const int ND = 2;
const int N_N = 4; //sides per element
const int Nfield = 4;
const int plus = 1;
...
const char* ux_file =   "ux0000000005.pos";
const char* uy_file =   "uy0000000005.pos";
const char* rho_file = "rho0000000005.pos";
const char* p_file =     "p0000000005.pos";
...
int main()
{
  ...
  */
  ...
  //For post-processing: Get the bounds of each element
  D2_double Bounds = Allocate_2D_double(RES_cells, D*2); //holds min and max
  for (int om = 0; om < RES_cells; om++){
    double xmin = pow(10,10); double ymin = pow(10,10);
    double xmax = -pow(10,10); double ymax = pow(-10,10);
    for (int k = 0; k < N_N; k++){
      xmin = fmin(xmin, CELLVERT[om][k].rx);
      xmax = fmax(xmax, CELLVERT[om][k].rx);
      ymin = fmin(ymin, CELLVERT[om][k].ry);
      ymax = fmax(ymax, CELLVERT[om][k].ry);
      Bounds[om][0] = xmin;
      Bounds[om][1] = xmax;
      Bounds[om][2] = ymin;
      Bounds[om][3] = ymax; }}

  //Also for post-processing: for starters, plot cell-averaged
  //data. This prevents me from having to build the polynomial
  //inside each element
  D2_double Ubar = Allocate_2D_double(RES_cells, Nfield);
  for (int om = 0; om < RES_cells; om++){
    D1_double Avg = Allocate_1D_double(Nfield);
    for (int k = 0; k < Nsolut; k++){
      for (int fc = 0; fc < Nfield; fc++){
	Avg[fc] += DOF[om][k][fc] / (Nsolut+0.0);}}
  ...
  //10/23/2017: Post-processing for workshoip shock-vortex problem. 
  //Looking for solution distribution along some well-defined lines
  double xLine[NL];
  double yLine[NL];
  double hLine = 2.0/NL;
  D2_double SolBarLine = Allocate_2D_double(NL, Nfield);
  for (int i = 0; i < NL; i++){
    xLine[i] = 0.5*hLine + (i-1)*hLine;
    yLine[i] = 0.4 + 0.0001; }
  //Line locations have been set. For each location,
  //identify the containing element.
  //This is relatively easy for quads, could be nightmarish
  //for simplex elements
  for (int i = 0; i < NL; i++)
    {
      int home = -1;
      for (int om = 0; e < RES_cells; e++)
	{
	  if (xLine[i] <= Bounds[om][1] && xLine[i] >= Bounds[om][0])
	    {
	      if (yLine[i] <= Bounds[om][3] && yLine[i] >= Bounds[om][2])
		{
		  //we have found the point's home element
		  home = om;
		  SolBarLine[i] = Ubar[home];
		}
	    }
	}
    }
  ...
  FILE*LineBar_XY = fopen("LineXY.csv","w");
  FILE*LineBar_rho = fopen("LineBarRHO.csv","w");
  FILE*LineBar_ux = fopen("LineBarVX.csv","w");
  FILE*LineBar_uy = fopen("LineBarVY.csv","w");
  FILE*LineBar_p = fopen("LineBarPRES.csv","w");
  for (int i = 0; i < NL; i++){
    fprintf(LineBar_XY, "%f,%f\n",xLine[i], yLine[i]);
    fprintf(LineBar_rho, "%f\n",SolBarLine[i][0]);
    fprintf(LineBar_ux, "%f\n",SolBarLine[i][1]);
    fprintf(LineBar_uy, "%f\n",SolBarLine[i][2]);
    fprintf(LineBar_p, "%f\n",SolBarLine[i][3]); }
