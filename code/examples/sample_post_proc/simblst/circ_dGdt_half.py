...
# circulation in half the domain (do this for the Vazsonyi equation too)
# 
#
__author__ = 'marchdf'
...
for k,timestep in enumerate(timesteps): 
    ...
    #
    nhalf = np.shape(X)[1]/2

    # Get circulation in the full domain
    t[k]  = time
    G[k]  = simps(simps(w[:,:nhalf],X[0,:nhalf]),Y[:,0])
    VG[k] = simps(simps(w_rho[:,:nhalf],X[0,:nhalf]),Y[:,0]) 

    # Get the dG/dt for the various terms in the vorticity equation
    dGdt_advective[k]    = simps(simps(advective[:,:nhalf],X[0,:nhalf]),Y[:,0])
    dGdt_compressible[k] = simps(simps(compressible[:,:nhalf],X[0,:nhalf]),Y[:,0])
    dGdt_baroclinic[k]   = simps(simps(baroclinic[:,:nhalf],X[0,:nhalf]),Y[:,0])

    dVGdt_advective[k]    = simps(simps(Vadvective[:,:nhalf],X[0,:nhalf]),Y[:,0])
    dVGdt_compressible[k] = simps(simps(Vcompressible[:,:nhalf],X[0,:nhalf]),Y[:,0])
    dVGdt_baroclinic[k]   = simps(simps(Vbaroclinic[:,:nhalf],X[0,:nhalf]),Y[:,0])
