...
from mytermcolor import colored
...
#
#================================================================================
BASEDIR = os.getcwd(); BASEDIR = BASEDIR.split("/code/")[0]+'/'
#print BASEDIR
CODEDIR = BASEDIR+'code/'
BINDIR  = CODEDIR+'bin/'
BINNAME = 'dgexec'
TESTDIR = CODEDIR+'tests/'
subdirs = get_immediate_subdirectories(TESTDIR); subdirs.sort();
...
for cnt,subdir in enumerate(subdirs):
    ...
    print '=' * 80,'\n'
    print ' Running test [{0:.0f}/{1:.0f}]'.format(cnt+1,num2test),'in directory',this_test_dir,'\n';
    print '=' * 80
    ...
    deckname = [deckname for deckname in os.listdir(this_test_dir) if deckname.endswith('.inp')]
    deckname = deckname[0]
...
print colored('Number of succesful tests = {0:2.0f}'.format(num2test-fails),successcolor)
