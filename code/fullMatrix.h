 ...
 public:
  ...
  inline scalar get(int r, int c) const
  {
    ...
    if (r >= _r || r < 0 || c >= _c || c < 0)
      printf("invalid index to access fullMatrix : %i %i (size = %i %i)", 
                 r, c, _r, _c);
    #endif
    return (*this)(r, c); 
  }
  ...
  void setAsShapeProxy(fullMatrix<scalar> &original, int nbRow, int nbCol)
  {
    ...
    if(_c*_r != original._c*original._r)
      printf("Trying to reshape a fullMatrix without conserving the "
  ...
  void operator += (const fullMatrix<scalar> &other)
  {
    if(_r != other._r || _c!= other._c)
      printf("sum matrices of different sizes\n");
    for(int i = 0; i < _r * _c; ++i) _data[i] += other._data[i];
  }
  inline scalar operator () (int i, int j) const
  {
    ...
    if (i >= _r || i < 0 || j >= _c || j < 0)
      printf("invalid index to access fullMatrix : %i %i (size = %i %i)",
                 i, j, _r, _c);
    #endif
    return _data[i + _r * j];  // column major!!
  }
  ...
  {
    if(size1() != size2()){
      printf("Not a square matrix (size1: %d, size2: %d)", size1(), size2());
    }
    scalar t;
    for(int i = 0; i < size1(); i++)
      for(int j = 0; j < i; j++) {
...
#if !defined(HAVE_LAPACK)
  {
    printf("Matrix inversion requires LAPACK");
    return false;
  }
#endif
  ;
...
#if !defined(HAVE_LAPACK)
  {
    printf("Determinant computation requires LAPACK");
    return 0.;
  }
#endif
  ;
...
#if !defined(HAVE_LAPACK)
  {
    printf("Singular value decomposition requires LAPACK");
    return false;
  }
#endif
  ;
  ...
  void print(const std::string name = "", const std::string fname = "") const 
  {
    printf("Printing matrix %s:\n", name.c_str());
    int ni = size1();
    int nj = size2();
