/*!
  \file printer_twod_multifluid.cu
  \brief Kernel to output 2D multifluid solution used by the PRINTER class
...
arch_global void formater(int N_s, int N_E, scalar* U, scalar* output, bool inverse){
  ...
  */
  ...
  if(!inverse){
...
#ifdef GAMCONS
	scalar gamma = 1+rho/U[(e*N_F+4)*N_s+i];
#elif  GAMNCON
	...
	output[(e*N_F+4)*N_s+i] = (gamma-1)*(et - 0.5*(ux*ux+uy*uy)*rho);
