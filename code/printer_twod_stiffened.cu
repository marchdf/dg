/*!
  \file printer_twod_stiffened.cu
  \brief Kernel to output 2D stiffened solution used by the PRINTER class
...
arch_global void formater(int N_s, int N_E, scalar* U, scalar* output, bool inverse){
  ...
  */
  ...
  if(!inverse){
	...
	scalar uy  = U[(e*N_F+2)*N_s+i]/rho;
	scalar et  = U[(e*N_F+3)*N_s+i];
	scalar gamma = 1+1.0/U[(e*N_F+4)*N_s+i];
	scalar pinf = (1-1.0/gamma)*U[(e*N_F+5)*N_s+i];
	
	output[(e*N_F+0)*N_s+i] = rho;
	output[(e*N_F+1)*N_s+i] = ux;
	output[(e*N_F+2)*N_s+i] = uy;
	output[(e*N_F+3)*N_s+i] = gamma;
	output[(e*N_F+4)*N_s+i] = pinf;
	output[(e*N_F+5)*N_s+i] = (gamma-1)*(et - gamma*pinf/(gamma-1) - 0.5*(ux*ux+uy*uy)*rho);
	
	// Mass fractions
#include "loopstart.h"
#define LOOP_END N_Y
#define MACRO(x) output[(e*N_F+6+x)*N_s+i] = U[(e*N_F+6+x)*N_s+i]/rho;
#include "loop.h"
      } // loop on nodes
    } // loop on elements
  } // if inverse
  ...
  //
	...
	scalar gamma = output[(e*N_F+3)*N_s+i];
	scalar pinf  = output[(e*N_F+4)*N_s+i];
	scalar p     = output[(e*N_F+5)*N_s+i];

	U[(e*N_F+0)*N_s+i] = rho;
	U[(e*N_F+1)*N_s+i] = rho*ux;
	U[(e*N_F+2)*N_s+i] = rho*uy;
	U[(e*N_F+3)*N_s+i] = p/(gamma-1) + gamma*pinf/(gamma-1) + 0.5*rho*(ux*ux+uy*uy);
	U[(e*N_F+4)*N_s+i] = 1.0/(gamma-1);
	U[(e*N_F+5)*N_s+i] = gamma*pinf/(gamma-1);

	// Mass fractions
#include "loopstart.h"
#define LOOP_END N_Y
#define MACRO(x) U[(e*N_F+6+x)*N_s+i] = rho*output[(e*N_F+6+x)*N_s+i];
#include "loop.h"
      } // loop on nodes
    }  // loop on elements
  } // if inverse
