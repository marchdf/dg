...
//==========================================================================
arch_global void limmodif(int N_s, int N_E, int slicenum, scalar* A, scalar* plim, scalar* Alim){


#ifdef ONED   // Eidx = energy index. in 1D: Eidx = 2. in 2D, Eidx = 3
  int Eidx = 2; int Gidx = 3;
#elif TWOD
  int Eidx = 3; int Gidx = 4;
#endif   // Gidx = gamma index. in 1D: Gidx = 3. in 2D, Gidx = 4
  ...
  if (e < N_E){
	...
	scalar d0 = Alim[(e*N_F+Gidx)*N_s*slicenum+slice*N_s+0];
	scalar d1 = Alim[(e*N_F+Gidx)*N_s*slicenum+slice*N_s+1];
#ifdef GAMCONS
...
#ifdef ONED
	scalar momentumP = (b0+b1)*(b0+b1);
	scalar momentumM = (b0-b1)*(b0-b1);
#elif TWOD // take into account both momentum components
	scalar bb0 = Alim[(e*N_F+2)*N_s*slicenum+slice*N_s+0];
	scalar bb1 = Alim[(e*N_F+2)*N_s*slicenum+slice*N_s+1];
	scalar momentumP = (b0+b1)*(b0+b1)+(bb0+bb1)*(bb0+bb1);
	scalar momentumM = (b0-b1)*(b0-b1)+(bb0-bb1)*(bb0-bb1);
#endif
	
	scalar g1lim = d1/d0*(g0-0.25*(momentumP/(a0+a1) + momentumM/(a0-a1))) + 0.25*(momentumP/(a0+a1) - momentumM/(a0-a1)) + plim[e*N_s*slicenum+slice*N_s+1]*d0;
	scalar g0lim = g0;
	Alim[(e*N_F+Eidx)*N_s*slicenum+slice*N_s+1] = g1lim;
	Alim[(e*N_F+Eidx)*N_s*slicenum+slice*N_s+0] = g0lim;
	...
	scalar g1lim = 0.25*(momentumP/(a0+a1) - momentumM/(a0-a1)) + plim[e*N_s*slicenum+slice*N_s+1]*gamma;
	scalar g0lim = g0;
	Alim[(e*N_F+Eidx)*N_s*slicenum+slice*N_s+1] = g1lim;
	Alim[(e*N_F+Eidx)*N_s*slicenum+slice*N_s+0] = g0lim;
#endif
	...
	scalar a0 = Alim[(e*N_F+0)*N_s*slicenum+slice*N_s+0];
	scalar a1 = Alim[(e*N_F+0)*N_s*slicenum+slice*N_s+1];
	scalar a2 = Alim[(e*N_F+0)*N_s*slicenum+slice*N_s+2];
	scalar b0 = Alim[(e*N_F+1)*N_s*slicenum+slice*N_s+0];
	scalar b1 = Alim[(e*N_F+1)*N_s*slicenum+slice*N_s+1];
	scalar b2 = Alim[(e*N_F+1)*N_s*slicenum+slice*N_s+2];
	scalar g0 = A[(e*N_F+Eidx)*N_s*slicenum+slice*N_s+0];
	scalar g1 = A[(e*N_F+Eidx)*N_s*slicenum+slice*N_s+1];
	scalar g2 = A[(e*N_F+Eidx)*N_s*slicenum+slice*N_s+2];
	scalar d0 = Alim[(e*N_F+Gidx)*N_s*slicenum+slice*N_s+0];
	scalar d1 = Alim[(e*N_F+Gidx)*N_s*slicenum+slice*N_s+1];
	scalar d2 = Alim[(e*N_F+Gidx)*N_s*slicenum+slice*N_s+2];
#ifdef GAMCONS
	scalar D0 = d0/a0;
...
#ifdef ONED
	scalar momentumP = (b0+b1+0.5*b2)*(b0+b1+0.5*b2);
	scalar momentumM = (b0-b1+0.5*b2)*(b0-b1+0.5*b2);
	scalar momentumX = b0*b0;
#elif TWOD // take into account both momentum components
	scalar bb0 = Alim[(e*N_F+2)*N_s*slicenum+slice*N_s+0];
	scalar bb1 = Alim[(e*N_F+2)*N_s*slicenum+slice*N_s+1];
	scalar bb2 = Alim[(e*N_F+2)*N_s*slicenum+slice*N_s+2];
	scalar momentumP = (b0+b1+0.5*b2)*(b0+b1+0.5*b2) + (bb0+bb1+0.5*bb2)*(bb0+bb1+0.5*bb2);
	scalar momentumM = (b0-b1+0.5*b2)*(b0-b1+0.5*b2) + (bb0-bb1+0.5*bb2)*(bb0-bb1+0.5*bb2);
	scalar momentumX = b0*b0 + bb0*bb0;
#endif
	
	scalar g2lim = (1/(1+1.0/6.0*d2/d0))
	  *(d2/d0*(g0+1.0/6.0*g2 -0.5*momentumX/a0)
	    + 0.5*(momentumP/(a0+a1+0.5*a2) + momentumM/(a0-a1+0.5*a2) - 2*momentumX/a0)
	    + plim[e*N_s*slicenum+slice*N_s+2]*d0);
	scalar g1lim = d1/d0*(g0+1.0/6.0*(g2-g2lim)-0.5*momentumX/a0)
	  + 0.25*(momentumP/(a0+a1+0.5*a2) - momentumM/(a0-a1+0.5*a2))
	  + plim[e*N_s*slicenum+slice*N_s+1]*d0;
	scalar g0lim = g0 + 1.0/6.0*(g2-g2lim);
	Alim[(e*N_F+Eidx)*N_s*slicenum+slice*N_s+0] = g0lim;
	Alim[(e*N_F+Eidx)*N_s*slicenum+slice*N_s+1] = g1lim;
	Alim[(e*N_F+Eidx)*N_s*slicenum+slice*N_s+2] = g2lim;
#elif PASSIVE
	scalar a0 = Alim[(e*N_F+0)*N_s*slicenum+slice*N_s+0];
	scalar a1 = Alim[(e*N_F+0)*N_s*slicenum+slice*N_s+1];
	scalar a2 = Alim[(e*N_F+0)*N_s*slicenum+slice*N_s+2];
	...
	scalar gamma = constants::GLOBAL_GAMMA;
	scalar g2lim = 0.5*(momentumP/(a0+a1+0.5*a2) + momentumM/(a0-a1+0.5*a2) - 2*momentumX/a0) + plim[e*N_s*slicenum+slice*N_s+2]*gamma;
	scalar g1lim = 0.25*(momentumP/(a0+a1+0.5*a2) - momentumM/(a0-a1+0.5*a2))
	  + plim[e*N_s*slicenum+slice*N_s+1]*gamma;
	scalar g0lim = g0 + 1.0/6.0*(g2-g2lim);
	Alim[(e*N_F+Eidx)*N_s*slicenum+slice*N_s+0] = g0lim;
	Alim[(e*N_F+Eidx)*N_s*slicenum+slice*N_s+1] = g1lim;
	Alim[(e*N_F+Eidx)*N_s*slicenum+slice*N_s+2] = g2lim;
...
extern "C"
void Llimmodif(int N_s, int N_E, int slicenum, scalar* A, scalar* plim, scalar* Alim){
#ifdef USE_GPU
  int div = N_E/blkE;
  int mod = 0;
  if (N_E%blkE != 0) mod = 1;
  ...
  limmodif arch_args (N_s, N_E, slicenum, A, plim, Alim);
}
